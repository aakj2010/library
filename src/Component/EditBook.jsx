...
function EditBook() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            await axios.put(`https://62fe35d041165d66bfbb1342.mockapi.io/Books/${params.id}`, values);
            alert("book Edited")
            navigate("/books")
        }
    });
    ...
    let loadBook = async () => {
        try {
            let book = await axios.get(`https://62fe35d041165d66bfbb1342.mockapi.io/Books/${params.id}`)
            formik.setValues({
                name: book.data.name,
